...
async function liveStats(site: string, all: boolean) {
  ...
  const [online, today, channels, referrers, pages, countries, series, aiRes, avg, bounce, devices, browsers, os, campaigns, prevToday, prevAvg, prevBounce] = await Promise.all([
    ...
    queryRows<OneRow>(`SELECT avg(m) AS d FROM (SELECT least(sum(duration_ms), 1800000) AS m FROM events WHERE duration_ms > 0 AND duration_ms <= 45000 AND ts >= toStartOfDay(now())${filter} GROUP BY visitor_id)`, params),
    ...
    queryRows<OneRow>(`SELECT avg(m) AS d FROM (SELECT least(sum(duration_ms), 1800000) AS m FROM events WHERE duration_ms > 0 AND duration_ms <= 45000 AND ts >= toStartOfDay(now()) - INTERVAL 1 DAY AND ts < now() - INTERVAL 1 DAY${filter} GROUP BY visitor_id)`, params),
...
async function eventsHistory(filter: string, params: Record<string, unknown> | undefined, days: number, base: Base, revMap: Record<string, RevenueBucket> | null, win: string, pwin: string) {
  ...
  const [tot, series, channels, referrers, pages, countries, devices, browsers, os, campaigns, avg, bounce, prevTot, prevAvg, prevBounce] = await Promise.all([
    ...
    queryRows<OneRow>(`SELECT avg(m) AS d FROM (SELECT least(sum(duration_ms), 1800000) AS m FROM events WHERE duration_ms > 0 AND duration_ms <= 45000 AND ${win}${filter} GROUP BY visitor_id)`, params),
    ...
    queryRows<OneRow>(`SELECT avg(m) AS d FROM (SELECT least(sum(duration_ms), 1800000) AS m FROM events WHERE duration_ms > 0 AND duration_ms <= 45000 AND ${pwin}${filter} GROUP BY visitor_id)`, params),
